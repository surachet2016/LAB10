...
const AdminPage = ({ route }) => {
  ...
  const [profilePic, setProfilePic] = useState(
    "http://mgt2.pnu.ac.th/Lab10/profile_pics/default.jpg"
  );
    ...
    try {
      ...
      if (result.assets && result.assets.length > 0) {
        ...
        const uploadResponse = await fetch(
          `http://mgt2.pnu.ac.th/Lab10/updateProfilePics.php?UserId=${UserId}`,
          {
            method: "POST",
            body: formData,
            headers: {
  ...
  const loadProfilePicture = async (userId) => {
    try {
      ...
      const response = await fetch(
        `http://mgt2.pnu.ac.th/Lab10/getProfilePics.php?UserId=${userId}`
      );
      const data = await response.json();
      if (data.success) {
        console.log(data.avatarUrl);
        const imageUrl = `http://mgt2.pnu.ac.th/Lab10/profile_pics/${
    ...
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={handleUpdatePicture}>
        {!loading && (
          <Image
            source={{
              uri: `http://mgt2.pnu.ac.th/Lab10/profile_pics/${UserId}_avatar.jpg?t=${new Date().toISOString()}`,
            }}
            style={styles.avatar}
            onError={(e) =>
